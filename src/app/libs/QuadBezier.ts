...
import { PlanarCurve } from "./PlanarCurve";

import { TSMT$Gauss, IEvalFcn } from "./Gauss";

import { TSMT$TWBRF } from "./TWBRF";

export class TSMT$QuadBezier extends PlanarCurve
{
  // quadratic polynomial coefficient values
  protected _c0X: number = 0;
  protected _c0Y: number = 0;
  protected _c1X: number = 0;
  protected _c1Y: number = 0;
  protected _c2X: number = 0;
  protected _c2Y: number = 0;

  protected _s: number = 0;         // cache recently requested normalized arc-length
  protected _t: number = 0;         // natural parameter at that normalized arc length;
  ...
  protected _integral: TSMT$Gauss = null;

  /**
   * Construct a new TSMT$QuadBezier
   *
   * @returns {nothing}
   */
  constructor()
   ...
   *
   * @param {IControlPoints} control
   */
  public fromObject(control: IControlPoints): void
  {
    this.x0 = control.x0;
    this.y0 = control.y0;
    this.cx = control.cx;
   ...
   *
   * @param {number} t Natural parameter value, normally in [0,1] but values outside that interval are allowed for extrapolation purposes
   *
   * @returns {number}
   */
  ...
  /**
   * Perform 3-point bezier interpolation, i.e. compute the geometric constraints that cause the quad bezier to pass through the three
   * specified points
   *
   * @param {Array<IPoint>} points Interpolation points
   *
   * @returns {number} Value of t for the middle interpolation point (obtained via chord-length parameterization)
   */
  ...
  public getTAtS(s: number): number
  {
    ...
    // check for duplicate request and edge cases
    if (s == this._s) {
      return this._t;
    }
    ...
    this._integral = this._integral || new TSMT$Gauss();
    ...
    const invFcn: IEvalFcn = (t: number): number => {
      return this._integral.eval((t: number) => this.__integrand(t), 0, t, 6)/this._length - this._normArcLength;
    };

    let t: number = TSMT$TWBRF.findRoot(left, right, invFcn);
    if (t >= 0 && t <= 1)
    {
      this._s = s;
      this._t = t;
  ...
  /**
   * Compute the y-coordinate(s) at the specified x-coordinate
   *
   * @param {number} x x-coordinate
   *
   * @returns {Array<number>} There could be as many as two y-values on the curve at the specified x-coordinate
   */
  public getYAtX(x: number): Array<number>
  {
    ...
    {
      let x1: number = this.getX(0);
      let x2: number = this.getX(1);
  ...
  /**
   * Compute the x-coordinate(s) at the specified y-coordinate
   *
   * @param {number} y y-coordinate
   *
   * @returns {Array<number>} There could be as many as two x-values on the curve at the specified y-coordinate
   */
  ...
  public sAtT(t: number): number
  {
    ...
    if (Math.abs(this._length) < 0.00001) {
      return 0;
    }

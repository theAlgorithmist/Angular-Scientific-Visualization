...
export class TSMT$Normal
{
  protected static readonly SQRT_2: number    = Math.sqrt(2);
  protected static readonly SQRT_2_PI: number = Math.sqrt(Math.PI + Math.PI);

  // mean and std. dev. of the distribution
  protected _mean: number = 0.0;
  protected _std: number  = 1.0;
  ...
  */
  public getNormal(x: number): number
  {
    ...
    const c: number = 1/(this._std*TSMT$Normal.SQRT_2_PI);
  ...
  */
  ...
  {
    let b: number, c: number, exp: number;

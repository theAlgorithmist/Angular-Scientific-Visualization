...
/**
 * Angular Dev Toolkit: Computations involving normal distributions
 *
 * @author Jim Armstrong (www.algorithmist.net)
 *
 * @version 1.0
 */
var TSMT$Normal = /** @class */ (function () {
    function TSMT$Normal() {
    ...
    Object.defineProperty(TSMT$Normal.prototype, "mean", {
        /**
         ...
         *
         * @param {number} value Mean for the normal distribution
         *
         * @returns {nothing}
         */
        set: function (value) {
            this._mean = !isNaN(value) ? value : this._mean;
        },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(TSMT$Normal.prototype, "std", {
        /**
    ...
    /**
     * Compute the probability that N(X) <= x (lower cumulative)
     *
     * @param {number} x Input value to test
     *
     * @returns {number}  P(N(X) <= x) or lower-cumulative dist; upper is one minus this value
     */
    TSMT$Normal.prototype.normaldist = function (x) {
        if (isNaN(x)) {
            return 0;
        }
        // normalize to unit
     ...
     *
     * @returns {number} x such that P(X<=x) = p.  Note that p MUST be strictly in the open interval (0,1).
     * You break it, you buy it.
     */
    TSMT$Normal.prototype.invnormaldist = function (p) {
     ...
     *
     * @returns {number} Probability that a sample from the current normal distribution lies within {n}
     * standard deviations of the mean.  The result is computed for a standard normal distribution.
     */
    TSMT$Normal.prototype.predictionInterval = function (n) {
        if (n <= 0) {
            return 0;
        }
     ...
     * sample from the distribution is expected to be inside that range with the supplied probability.
     *
     * @param {number} p Probability - must be in (0,1)
     *
     * @returns {number} If u is the mean and s is the standard deviation, then X is expected to be in u +/-
     * ns with probability, p.
     */
    TSMT$Normal.prototype.inversePredictionInterval = function (p) {
        if (p <= 0 || p >= 1) {
            return NaN;
        }
        return TSMT$Normal.SQRT_2 * this.__invERF(p);
    };
    /**
     ...
     *
     * @returns {number} N(x) or normal distribution function (not CDF) evaluated at x
     */
    TSMT$Normal.prototype.getNormal = function (x) {
        var z = (x - this._mean) / this._std;
        var d = -0.5 * z * z;
        var c = 1 / (this._std * TSMT$Normal.SQRT_2_PI);
        return c * Math.exp(d);
    };
    /**
     ...
     * @param {number} x x-coordinate
     *
     * @returns {number} dN(x)/dx
     */
    TSMT$Normal.prototype.getNormalDerivative = function (x) {
        var s = this._std * this._std;
        var f = this.getNormal(x);
    ...
    TSMT$Normal.prototype.__normalCDF = function (x) {
        var b, c, exp;
        // Hart approximation
        var zAbs = Math.abs(x);
        if (zAbs > 37) {
    ...
    TSMT$Normal.prototype.__invNormalCDF = function (p) {
    ...
    TSMT$Normal.prototype.__invERF = function (x) {
    ...
    TSMT$Normal.SQRT_2 = Math.sqrt(2);
    TSMT$Normal.SQRT_2_PI = Math.sqrt(Math.PI + Math.PI);
    return TSMT$Normal;
}());
exports.TSMT$Normal = TSMT$Normal;
